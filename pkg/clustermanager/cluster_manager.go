...
// WithNoTimeout disables the timeout for all the waits and retries in cluster manager.
func WithNoTimeout() ClusterManagerOpt {
	return func(c *ClusterManager) {
		noTimeoutRetrier := retrier.NewWithNoTimeout()
		maxTime := time.Duration(math.MaxInt64)

		c.clusterClient.Retrier = noTimeoutRetrier
